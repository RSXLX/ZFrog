...
const resolveArtifactPaths = ({ reportsDir, runId, args }) => {
  ...
  return {
    ...
    closeoutPreviewPath:
      toTrimmedString(args['closeout-preview']) ||
      path.join(prefix, `v3-dashboard-freeze-rollback-drill-${runId}-backlog-closeout.md`)
  };
};

const assertArtifactPathsMatchExpected = ({ runId, reportsDir, artifactPaths }) => {
  const expectedPaths = resolveV3DashboardDrillExpectedArtifactPaths({
    reportsDir,
    runId
  });
  const expectedCloseoutPreviewPath = path.join(
    resolveV3DashboardDrillReportsDir(reportsDir),
    `v3-dashboard-freeze-rollback-drill-${runId}-backlog-closeout.md`
  );
  ...
  const checks = [
    ...
    {
      label: 'closeout-preview',
      actual: artifactPaths.closeoutPreviewPath,
      expected: expectedCloseoutPreviewPath
    }
  ];
  ...
  for (const check of checks) {
    if (
      ...
      throw new Error(`Unexpected --${check.label} path: expected ${check.expected}, got ${check.actual}`);
    }
  }
};
...
const main = async () => {
  ...
  const [
    ...
    readFile(artifactPaths.closeoutPreviewPath, 'utf8'),
    readFile(backlogDocPath, 'utf8')
  ]);

  const applyResult = applyV3DashboardDrillCloseoutPreview({
    runId,
    taskId,
    reportsDir,
    ...
    backlogDoc,
    closeoutPreviewDoc: closeoutPreviewContent,
  ...
  await writeFile(backlogDocPath, applyResult.appliedContent, 'utf8');

  process.stdout.write(
    [
      `Closeout preview applied for task ${applyResult.taskId}`,
      `runId=${applyResult.runId}`,
      `publishable=${applyResult.evidenceResult.publishable ? 'yes' : 'no'}`,
      `readyToClose=${applyResult.evidenceResult.readyToClose ? 'yes' : 'no'}`,
      `backlogDoc=${backlogDocPath}`
    ].join('\n') + '\n'
  );
};
...
main().catch((error) => {
  ...
  process.stderr.write(`[v3-dashboard-drill-closeout-apply] ${message}\n`);
  process.exit(1);
});
