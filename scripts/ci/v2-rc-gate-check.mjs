...
import { evaluateGate, normalizeRuns, renderMarkdownReport } from './v2-rc-gate-lib.mjs';

const DEFAULT_CONFIG_PATH = '.github/release-gates/v2-rc-gate.json';
...
const fetchNightlyRuns = async ({ config, repository, token, apiBase }) => {
  ...
  const sampleSize = Number(nightly.sampleSize ?? 12);
...
const main = async () => {
  ...
  if (args['runs-fixture']) {
    ...
    runPayload = await fetchNightlyRuns({ config, repository, token, apiBase });
    runSource = `github:${repository}`;
  }

  const runs = normalizeRuns(runPayload);
  const result = evaluateGate({ config, runs, now });
...
main().catch((error) => {
  ...
  process.stderr.write(`[v2-rc-gate] ${message}\n`);
  process.exit(1);
});
