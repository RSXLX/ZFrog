...
import { renderArchivedReport, resolveArchiveFilename } from './v3-rc-gate-archive-lib.mjs';

const DEFAULT_REPORT_PATH = 'reports/v3/v3-rc-gate-report.md';
const DEFAULT_ARCHIVE_DIR = 'reports/history/v3-rc-gate';
...
const main = async () => {
  ...
  process.stdout.write(`[v3-rc-gate-archive] archived report -> ${archivePath}\n`);
  process.stdout.write(`[v3-rc-gate-archive] updated latest -> ${latestPath}\n`);
};

main().catch((error) => {
  const message = error instanceof Error ? error.message : String(error);
  process.stderr.write(`[v3-rc-gate-archive] ${message}\n`);
  process.exit(1);
});
