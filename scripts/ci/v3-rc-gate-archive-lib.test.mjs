...
import {
  ...
  sanitizeFileToken,
  toCompactUtcTimestamp
} from './v3-rc-gate-archive-lib.mjs';

test('toCompactUtcTimestamp renders sortable utc value', () => {
  const value = toCompactUtcTimestamp(new Date('2026-03-24T10:11:12.000Z'));
  assert.equal(value, '20260324T101112Z');
});
...
test('resolveArchiveFilename includes run identifier and timestamp', () => {
  ...
  assert.equal(filename, 'v3-rc-gate-20260324T101112Z-run-9988.md');
});
...
test('renderArchivedReport prepends evidence metadata', () => {
  const report = renderArchivedReport({
    report: '# Base Report\n\n- Verdict: PASS\n',
    metadata: {
      archivedAt: '2026-03-24T10:11:12.000Z',
      workflow: 'v3-rc-release-gate',
      runId: '9988',
  ...
  assert.match(report, /# V3 RC Gate Archived Evidence/);
