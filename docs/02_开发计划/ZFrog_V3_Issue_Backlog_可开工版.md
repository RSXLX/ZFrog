...
5. `2026-03-23` 执行记录：
   ...
   39. `2026-03-24`：`V3-RC-02` 已补齐“commit 身份强绑定”安全闭环：升级 `scripts/ci/v3-dashboard-drill-evidence-gate*.mjs`、`scripts/ci/v3-dashboard-drill-closeout*.mjs`、`scripts/ci/v3-dashboard-drill-closeout-apply*.mjs` 新增 `--expected-ref` / `--expected-sha`（默认绑定 `GITHUB_REF/GITHUB_SHA`），并将其接入 `.github/workflows/v3-beta-regression-matrix.yml` 的 `evidence-gate` / `closeout-preview` 与 step summary 命令模板；当 `run-manifest.ref/sha` 与当前执行上下文不一致时一律 fail-closed，防止历史 commit 证据误用于正式关单。
   40. `2026-03-24`：`V3-RC-02` 已补齐“publishable 显式身份入参必填”安全闭环：升级 `scripts/ci/v3-dashboard-drill-evidence-gate-lib.mjs`，当结果为 publishable 时，`--expected-server-url/--expected-repository/--expected-run-attempt/--expected-ref/--expected-sha` 任一缺失即 fail-closed（不再允许静默 skip）；并补齐 `scripts/ci/v3-dashboard-drill-evidence-gate-lib.test.mjs`、`scripts/ci/v3-dashboard-drill-closeout-lib.test.mjs`、`scripts/ci/v3-dashboard-drill-closeout-apply-lib.test.mjs` 回归覆盖。
   41. `2026-03-24`：`V3-RC-02` 已补齐“commit sha 规范化强绑定”安全闭环：升级 `scripts/ci/v3-dashboard-drill-evidence-gate-lib.mjs`，publishable 场景强制 `expectedSha` 与 `run-manifest.sha` 必须为 40 位 git commit hash（短 sha/非十六进制一律 fail-closed）；并补齐 `scripts/ci/v3-dashboard-drill-evidence-gate-lib.test.mjs`、`scripts/ci/v3-dashboard-drill-closeout-lib.test.mjs`、`scripts/ci/v3-dashboard-drill-closeout-apply-lib.test.mjs` 回归覆盖。
   42. 下一点：在可开放端口环境（CI/staging）补跑 `test:e2e:admin:v3-dashboard`，拿到 workflow run 链接后校验 `evidence-gate` 报告为 PASS、`readyToClose=yes` 且自动生成的 `backlog-closeout` 预览存在，再执行带完整 `expected-*` 五元组（`server-url/repository/run-attempt/ref/sha`）的 `closeout-apply` 正式关单 `V3-RC-02`（若仍失败则保持 `暂缓发布` 并继续回滚收口）。
...
8. 执行记录（`2026-03-24`）：
   ...
   41. 已交付（`2026-03-24`）：补齐“commit 身份强绑定”安全闭环：升级 `scripts/ci/v3-dashboard-drill-evidence-gate-lib.mjs`（新增 `manifest ref/sha` 对 `--expected-ref/--expected-sha` 的 fail-closed 校验与报告字段）、`scripts/ci/v3-dashboard-drill-evidence-gate.mjs`、`scripts/ci/v3-dashboard-drill-closeout*.mjs`、`scripts/ci/v3-dashboard-drill-closeout-apply*.mjs`（新增 CLI 参数透传，默认绑定 `GITHUB_REF/GITHUB_SHA`），并将该参数接入 `.github/workflows/v3-beta-regression-matrix.yml` 的 `evidence-gate` / `closeout-preview` 与 step summary 命令模板，防止历史 commit 证据误关单。
   42. 验证闭环（本轮新增）：
      1. `node --test ./scripts/ci/v3-dashboard-drill-evidence-gate-lib.test.mjs ./scripts/ci/v3-dashboard-drill-closeout-lib.test.mjs ./scripts/ci/v3-dashboard-drill-closeout-apply-lib.test.mjs` 通过（30/30，新增 manifest ref/sha mismatch 负例）。
      2. `node ./scripts/ci/v3-dashboard-drill-evidence-gate.mjs --run-id local --reports-dir ./reports/v3 --task-id V3-RC-02 --run-manifest ./reports/v3/v3-dashboard-freeze-rollback-drill-local-run-manifest.json --expected-server-url https://github.com --expected-repository RSXLX/ZFrog --expected-run-attempt 1 --expected-ref local --expected-sha local --max-manifest-age-hours 72 --report /tmp/v3-dashboard-local-evidence-gate-with-refsha.md --out-json /tmp/v3-dashboard-local-evidence-gate-with-refsha.json` 通过（`passed=true`、`readyToClose=false`、`manifestRef=local`、`manifestSha=local`）。
      3. `node ./scripts/ci/v3-dashboard-drill-closeout.mjs --run-id local --reports-dir ./reports/v3 --run-manifest ./reports/v3/v3-dashboard-freeze-rollback-drill-local-run-manifest.json --backlog-doc /tmp/v3-rc02-refsha-apply-base.md --out-doc ./reports/v3/v3-dashboard-freeze-rollback-drill-local-backlog-closeout.md --expected-server-url https://github.com --expected-repository RSXLX/ZFrog --expected-run-attempt 1 --expected-ref local --expected-sha local --max-manifest-age-hours 72 --require-publishable false` 与 `node ./scripts/ci/v3-dashboard-drill-closeout-apply.mjs --run-id local --reports-dir ./reports/v3 --run-manifest ./reports/v3/v3-dashboard-freeze-rollback-drill-local-run-manifest.json --closeout-preview ./reports/v3/v3-dashboard-freeze-rollback-drill-local-backlog-closeout.md --backlog-doc /tmp/v3-rc02-refsha-apply-base.md --expected-server-url https://github.com --expected-repository RSXLX/ZFrog --expected-run-attempt 1 --expected-ref local --expected-sha local --max-manifest-age-hours 72 --require-publishable false` 通过（ref/sha gate 与 apply gate 可重放）。
   43. 已交付（`2026-03-24`）：补齐“publishable 显式身份入参必填”安全闭环：升级 `scripts/ci/v3-dashboard-drill-evidence-gate-lib.mjs`，当证据为 publishable 时，`expectedServerUrl/expectedRepository/expectedRunAttempt/expectedRef/expectedSha` 任一缺失即 fail-closed（不再允许 identity 校验静默 skip）；并同步更新 closeout/closeout-apply 相关单测入参，保持 strict 关单链路一致。
   44. 验证闭环（本轮新增）：
      1. `node --test ./scripts/ci/v3-dashboard-drill-evidence-gate-lib.test.mjs ./scripts/ci/v3-dashboard-drill-closeout-lib.test.mjs ./scripts/ci/v3-dashboard-drill-closeout-apply-lib.test.mjs` 通过（31/31，新增 publishable 缺 expected-* 入参负例）。
   45. 已交付（`2026-03-24`）：补齐“commit sha 规范化强绑定”安全闭环：升级 `scripts/ci/v3-dashboard-drill-evidence-gate-lib.mjs`，新增 publishable 场景下 `expectedSha` 与 `manifest.sha` 的 40 位 git commit hash 规范化校验（短 sha/非十六进制 fail-closed），并将该约束透传至 closeout/closeout-apply 关单链路。
   46. 验证闭环（本轮新增）：
      1. `node --test ./scripts/ci/v3-dashboard-drill-evidence-gate-lib.test.mjs ./scripts/ci/v3-dashboard-drill-closeout-lib.test.mjs ./scripts/ci/v3-dashboard-drill-closeout-apply-lib.test.mjs` 通过（33/33，新增 expectedSha 非规范与 manifest.sha 非规范负例）。
   47. 已交付（`2026-03-24`）：补齐“commit ref 规范化强绑定”安全闭环：升级 `scripts/ci/v3-dashboard-drill-evidence-gate-lib.mjs`，新增 publishable 场景下 `expectedRef` 与 `manifest.ref` 的 canonical full git ref 校验（必须为 `refs/...`，短分支名或非标准 ref 一律 fail-closed），并将该约束透传至 closeout/closeout-apply 关单链路。
   48. 验证闭环（本轮新增）：
      1. `node --test ./scripts/ci/v3-dashboard-drill-evidence-gate-lib.test.mjs ./scripts/ci/v3-dashboard-drill-closeout-lib.test.mjs ./scripts/ci/v3-dashboard-drill-closeout-apply-lib.test.mjs` 通过（37/37，新增 expectedRef 非 canonical 与 manifest.ref 非 canonical 负例）。
   49. 已交付（`2026-03-24`）：补齐“serverUrl origin-only 规范化强绑定”安全闭环：升级 `scripts/ci/v3-dashboard-drill-run-manifest-lib.mjs` 与 `scripts/ci/v3-dashboard-drill-evidence-gate-lib.mjs`，新增 `manifest.serverUrl` 与 `expectedServerUrl` 的 canonical origin 校验（必须为 `https://<host>`，禁止 path/query/hash/credentials），避免仅 host 命中但 URL 语义漂移导致的身份误判。
   50. 验证闭环（本轮新增）：
      1. `node --test ./scripts/ci/v3-dashboard-drill-run-manifest-lib.test.mjs ./scripts/ci/v3-dashboard-drill-evidence-gate-lib.test.mjs ./scripts/ci/v3-dashboard-drill-closeout-lib.test.mjs ./scripts/ci/v3-dashboard-drill-closeout-apply-lib.test.mjs` 通过（48/48，新增 expectedServerUrl path 与 manifest.serverUrl path 负例）。
   51. 已交付（`2026-03-24`）：完成 `V3-RC-02` publishable 实跑前提核对：远端 `RSXLX/ZFrog` 默认分支当前缺失 `.github/workflows/v3-beta-regression-matrix.yml`（raw/contents API 均为 404），且本地 V3 CI 关键产物（workflow/release-gates/`scripts/ci/v3-*`/`scripts/ci/v3-dashboard-drill-*`）仍未纳入 git，因此无法在当前远端触发可追溯 publishable run。
   52. 验证闭环（本轮新增）：
      1. `gh auth status -h github.com` 显示未登录；`curl -sS https://raw.githubusercontent.com/RSXLX/ZFrog/main/.github/workflows/v3-beta-regression-matrix.yml` 返回 `404`；`curl -sS https://api.github.com/repos/RSXLX/ZFrog/actions/workflows` 未返回该 workflow；`git push --dry-run origin main` 可用（说明 push 凭证存在，但仍需先发布 V3 CI 产物）。
9. 下一点执行清单（`2026-03-24`）：
   ...
   18. 已完成（`2026-03-24`）：`v3-beta-regression-matrix` workflow 与关单脚本链路已接入 `--expected-ref "${GITHUB_REF}"` + `--expected-sha "${GITHUB_SHA}"` 强约束，并完成本地 `ref/sha` 约束 rehearsal。
   19. 已完成（`2026-03-24`）：publishable 关单链路已升级为 `expected-*` 五元组必填（`server-url/repository/run-attempt/ref/sha`），缺任一参数即 fail-closed，避免人工执行时身份校验被静默跳过。
   20. 已完成（`2026-03-24`）：publishable `expectedSha` 与 `manifest.sha` 已升级为 40 位 git commit hash 规范化强校验（短 sha/非十六进制 fail-closed），并完成本地 `evidence-gate -> closeout -> closeout-apply` 回归验证。
   21. 已完成（`2026-03-24`）：publishable `expectedRef` 与 `manifest.ref` 已升级为 canonical full git ref 强校验（必须为 `refs/...`），并完成本地 `evidence-gate -> closeout -> closeout-apply` 回归验证。
   22. 已完成（`2026-03-24`）：publishable 身份链路已补齐 `expectedServerUrl` 与 `manifest.serverUrl` 的 canonical origin 强校验（禁止 path/query/hash/credentials），并完成本地 run-manifest/evidence-gate/closeout/closeout-apply 回归验证。
   23. 在可开放端口环境触发一次 `v3-beta-regression-matrix`（schedule 或 workflow_dispatch 均可），获取 run URL 与同 run 的 `run-manifest`。
   24. 校验 `reports/v3/v3-dashboard-freeze-rollback-drill-<runId>-evidence-gate.md` 对账 verdict 为 `PASS`，且 `readyToClose=yes`（manifest freshness/workflow identity/artifact digests/path scope/runUrl canonical（含 `https` / 无 query/hash）/repository identity（含 `--expected-repository`）/server host identity（含 origin-only `--expected-server-url`）/runAttempt identity（含 `--expected-run-attempt`）/commit identity（含 `--expected-ref` + `--expected-sha`，其中 ref 必须为 `refs/...` canonical full ref 且 sha 必须为 40 位 commit hash）均必须 PASS，必要时使用 `--require-publishable` + `--expected-server-url` + `--expected-repository` + `--expected-run-attempt` + `--expected-ref` + `--expected-sha` 复核）。
   25. 待执行：在可开放端口环境拿到 publishable run 后执行 `node ./scripts/ci/v3-dashboard-drill-closeout-apply.mjs --run-id <runId> --reports-dir ./reports/v3 --run-manifest ./reports/v3/v3-dashboard-freeze-rollback-drill-<runId>-run-manifest.json --closeout-preview ./reports/v3/v3-dashboard-freeze-rollback-drill-<runId>-backlog-closeout.md --backlog-doc ./docs/02_开发计划/ZFrog_V3_Issue_Backlog_可开工版.md --expected-server-url \"${GITHUB_SERVER_URL}\" --expected-repository \"${GITHUB_REPOSITORY}\" --expected-run-attempt \"${GITHUB_RUN_ATTEMPT}\" --expected-ref \"${GITHUB_REF}\" --expected-sha \"${GITHUB_SHA}\" --max-manifest-age-hours 72 --require-publishable true` 正式落库关闭 `V3-RC-02`；若 preview 一致性或任一 gate 失败则维持 `暂缓发布` 并继续回滚收口。
   26. 已完成（`2026-03-24`）：完成 `V3-RC-02` 实跑前提核对并确认当前阻塞为“远端未发布 V3 CI artifacts，暂不可触发 publishable workflow run”。
   27. 下一点：先将 V3 CI artifacts（workflow/release-gates/`scripts/ci/v3-*`/`scripts/ci/v3-dashboard-drill-*` 及其依赖）发布到远端分支并可触发运行，再执行第 23-25 条拿到同 run publishable 证据并落库关闭 `V3-RC-02`。
