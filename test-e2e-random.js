...
const { ZETAFROG_ABI } = require('./frontend/src/config/contracts');
...
async function testE2ERandomExploration() {
    ...
    try {
        ...
        console.log('📊 旅行信息:');
        console.log(`  - 目标地址: ${activeTravel.targetWallet}`);
        console.log(`  - 是否为零地址: ${activeTravel.targetWallet === zeroAddress}`);
        console.log(`  - 目标链ID: ${activeTravel.targetChainId}`);
        console.log(`  - 已完成: ${activeTravel.completed}`);
        console.log(`  - 开始时间: ${new Date(Number(activeTravel.startTime) * 1000).toISOString()}`);
        console.log(`  - 结束时间: ${new Date(Number(activeTravel.endTime) * 1000).toISOString()}`);
