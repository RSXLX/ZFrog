...
export const isMemoryWorldBetaEnabled = (): boolean => {
  const fromWindow =
    typeof window !== 'undefined'
      ? (window as unknown as { __ZFROG_V3_MEMORY_WORLD_BETA__?: unknown })
          .__ZFROG_V3_MEMORY_WORLD_BETA__
      : undefined;
...
export const isMemoryWorldOwnerEntryEnabled = (): boolean => {
  const fromWindow =
    typeof window !== 'undefined'
      ? (window as unknown as { __ZFROG_V3_MEMORY_WORLD_OWNER__?: unknown })
          .__ZFROG_V3_MEMORY_WORLD_OWNER__
      : undefined;
  const parsedWindow = readBooleanFlag(fromWindow);
  if (parsedWindow !== null) {
    return parsedWindow;
...
export const getMemoryWorldIntegrationApiKeySeed = (): string | null => {
  const fromWindow =
    typeof window !== 'undefined'
  ...
  const fromProcess =
    typeof process !== 'undefined' && process.env
      ? process.env.VITE_V3_MEMORY_WORLD_INTEGRATION_API_KEY || process.env.VITE_V3_INTEGRATION_API_KEY
      : undefined;
