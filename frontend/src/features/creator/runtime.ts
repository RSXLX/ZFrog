...
export const isCreatorBetaEnabled = (): boolean => {
  const fromWindow =
    typeof window !== 'undefined'
      ? (window as unknown as { __ZFROG_V3_CREATOR_BETA__?: unknown }).__ZFROG_V3_CREATOR_BETA__
      : undefined;
  ...
  const fromProcess =
    typeof process !== 'undefined' && process.env
      ? process.env.VITE_V3_CREATOR_BETA_ENABLED
      : undefined;
  return readBooleanFlag(fromProcess) ?? false;
};

export const getCreatorIntegrationApiKeySeed = (): string | null => {
  const fromWindow =
    typeof window !== 'undefined'
      ? (
          window as unknown as {
            __ZFROG_V3_CREATOR_INTEGRATION_API_KEY__?: unknown;
            __ZFROG_V3_INTEGRATION_API_KEY__?: unknown;
          }
        ).__ZFROG_V3_CREATOR_INTEGRATION_API_KEY__ ??
        (
          window as unknown as {
            __ZFROG_V3_CREATOR_INTEGRATION_API_KEY__?: unknown;
            __ZFROG_V3_INTEGRATION_API_KEY__?: unknown;
  ...
  const parsedImportMeta = readStringValue(
    import.meta.env.VITE_V3_CREATOR_INTEGRATION_API_KEY || import.meta.env.VITE_V3_INTEGRATION_API_KEY
  );
  if (parsedImportMeta) {
    return parsedImportMeta;
  }
  ...
  const fromProcess =
    typeof process !== 'undefined' && process.env
      ? process.env.VITE_V3_CREATOR_INTEGRATION_API_KEY || process.env.VITE_V3_INTEGRATION_API_KEY
      : undefined;
