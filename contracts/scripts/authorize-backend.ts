...
async function main() {
  ...
  // 检查当前授权状态
  try {
    const isAuthorized = await omniTravel.travelManagers(BACKEND_WALLET);
    if (isAuthorized) {
  ...
  try {
    const tx = await omniTravel.setTravelManager(BACKEND_WALLET, true);
